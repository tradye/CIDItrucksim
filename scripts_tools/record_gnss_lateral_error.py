...
class RecordGnssLateralError(object):
    """
    ...
    def __init__(self, record_file):
        ...
        self.logger = Logger.get_logger("RecordGnssLateralError")
        ...
        self.write("control_time,x,y,lateral_error,heading_error\n")  #
    ...
    def localization_callback(self, data):
        ...
        if self.startmoving:
            ...
            if self.auto_drive_status:
                ...
                self.write(
                    "%.4f, %.4f, %.4f,%.4f,%.4f\n" %
                    (self.control_time, carx, cary, self.lateral_error, self.heading_error))
            else:
...
def main(argv):
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    Logger.config(
        log_file=log_dir + "record_gnss_latera_lerror.log",
        use_stdout=True,
        log_level=logging.DEBUG)
    print("runtime log is in %s%s" % (log_dir, "record_gnss_latera_lerror.log"))
    record_file = log_dir + "/record_gnss_latera_lerror.csv"
    recorder = RecordGnssLateralError(record_file)
