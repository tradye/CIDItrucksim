...
class RecordPlotForMPCTest(object):
    """
    ...
    def __init__(self, record_file):
        ...
        #            "trailer_lateral_error,trailer_heading_error\n")#plan_x,plan_y
        ...
        self.trailer_lateral_error = 0.0
        self.trailer_heading_error = 0.0
    ...
    def control_callback(self,data):
        ...
        self.lateral_error = self.control_cmd.debug.truck_trailer_debug.lateral_error
        self.heading_error = self.control_cmd.debug.truck_trailer_debug.heading_error
        self.trailer_lateral_error = self.control_cmd.debug.truck_trailer_debug.trailer_lateral_error
        self.trailer_heading_error = self.control_cmd.debug.truck_trailer_debug.trailer_heading_error
    ...
    def localization_callback(self, data):
        ...
        if self.startmoving:
            ...
            # self.write(
            #     "%s, %s, %.4f, %s, %.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n" %
            #     (carx, cary,  self.control_cmd.debug.truck_trailer_debug.heading, carspeed, self.control_time,
            #      self.control_cmd.debug.truck_trailer_debug.steer_angle, self.lateral_error, self.heading_error,
            #      self.trailer_lateral_error, self.trailer_heading_error))
            self.write(
                "%.4f, %.4f, %.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n" %
                (self.control_time, self.control_cmd.debug.truck_trailer_debug.steer_angle, self.control_cmd.debug.truck_trailer_debug.steer_angle_feedback,
                 self.control_cmd.debug.truck_trailer_debug.steer_angle_feedforward, self.lateral_error, self.heading_error,
                 self.trailer_lateral_error, self.trailer_heading_error))
            self.logger.debug(
...
def main(argv):
    ...
    print("runtime log is in %s%s" % (log_dir, "record_for_mpc_test.log"))
    record_file = log_dir + "/record_for_mpc_test.csv"
    recorder = RecordPlotForMPCTest(record_file)
