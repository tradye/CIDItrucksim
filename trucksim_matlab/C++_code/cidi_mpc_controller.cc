...
#include "modules/control/controller/cidi_mpc_controller.h"
...
CIDIMPCController::CIDIMPCController() : name_("MPC-based Lateral Controller") {
...
CIDIMPCController::~CIDIMPCController() { CloseLogFile(); }

bool CIDIMPCController::LoadControlConf(const ControlConf *control_conf) {
  if (!control_conf) {
...
void CIDIMPCController::ProcessLogs(const SimpleLateralDebug *debug,
...
void CIDIMPCController::LogInitParameters() {
  AINFO << name_ << " begin.";
...
void CIDIMPCController::InitializeFilters(const ControlConf *control_conf) {
  // Low pass filter
...
Status CIDIMPCController::Init(const ControlConf *control_conf) {
...
void CIDIMPCController::CloseLogFile() {
  if (FLAGS_enable_csv_debug && steer_log_file_.is_open()) {
    steer_log_file_.close();
  }
}

void CIDIMPCController::LoadLatGainScheduler(
...
void CIDIMPCController::Stop() { CloseLogFile(); }

std::string CIDIMPCController::Name() const { return name_; }

Status CIDIMPCController::ComputeControlCommand(
...
Status CIDIMPCController::Reset() {
  previous_heading_error_ = 0.0;
  previous_lateral_error_ = 0.0;
  return Status::OK();
}

void CIDIMPCController::UpdateStateAnalyticalMatching(SimpleLateralDebug *debug) {
  ...
  matrix_state_(4, 0) = debug->lateral_error();//debug->trailer_lateral_error();
  matrix_state_(5, 0) = debug->lateral_error_rate();//debug->trailer_lateral_error_rate();
  matrix_state_(6, 0) = debug->heading_error();//debug->trailer_heading_error();
  matrix_state_(7, 0) = debug->heading_error_rate();//debug->trailer_heading_error_rate();
...
void CIDIMPCController::UpdateMatrix() {
  const double v =
...
void CIDIMPCController::UpdateMatrixCompound() {
...
double CIDIMPCController::ComputeFeedForward(double ref_curvature) const {
  const double kv =
 ...
 */
double CIDIMPCController::GetLateralError(const common::math::Vec2d &point,
...
void CIDIMPCController::ComputeLateralErrors(
