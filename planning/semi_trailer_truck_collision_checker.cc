...
#include "modules/planning/constraint_checker/semi_trailer_truck_collision_checker.h"
...
SemiTrailerTruckCollisionChecker::SemiTrailerTruckCollisionChecker(
...
bool SemiTrailerTruckCollisionChecker::InCollision(
     const DiscretizedTrajectory& discretized_truck_trajectory,
     const DiscretizedTrajectory& discretized_trailer_trajectory) {
  ...
  double trailer_length = vehicle_config.vehicle_param().trailer_length();
  double trailer_width = vehicle_config.vehicle_param().trailer_width();

  for (std::size_t i = 0; i < discretized_trajectory.NumOfPoints(); ++i) {
    const auto& truck_trajectory_point = discretized_truck_trajectory.TrajectoryPointAt(i);
    double ego_theta = truck_trajectory_point.path_point().theta();
    Box2d ego_box(
        {truck_trajectory_point.path_point().x(), truck_trajectory_point.path_point().y()},
    ...
    const auto& trailer_trajectory_point = discretized_trailer_trajectory.TrajectoryPointAt(i);
    double trailer_theta = trailer_trajectory_point.path_point().theta();
    Box2d trailer_box(
        {trailer_trajectory_point.path_point().x(), trailer_trajectory_point.path_point().y()},
        trailer_theta, trailer_length, trailer_width);
    double trailer_shift_distance =
        trailer_length / 2.0 - vehicle_config.vehicle_param().trailer_back_edge_to_center();
    Vec2d trailer_shift_vec{shift_distance * std::cos(trailer_theta),
			    shift_distance * std::sin(trailer_theta)};
    trailer_box.Shift(trailer_shift_vec);

    for (const auto& obstacle_box : predicted_bounding_rectangles_[i]) {
     if (ego_box.HasOverlap(obstacle_box) || trailer_box.HasOverlap(obstacle_box)) {
        return true;
      }
    }
  }
...
void SemiTrailerTruckCollisionChecker::BuildPredictedEnvironment(
    const std::vector<const Obstacle*>& obstacles, const double ego_vehicle_s,
    const double ego_vehicle_d,
    const std::vector<PathPoint>& discretized_reference_line) {
...
bool SemiTrailerTruckCollisionChecker::IsEgoVehicleInLane(const double ego_vehicle_d) {
  return std::fabs(ego_vehicle_d) < FLAGS_default_reference_line_width * 0.5;
}

bool SemiTrailerTruckCollisionChecker::ShouldIgnore(
